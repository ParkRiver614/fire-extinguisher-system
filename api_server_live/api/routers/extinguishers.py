...
from models.models import (
    Extinguisher, ExtinguisherModel, FloorNode, Zone, Floor, Building,
    SensorLog, SensorData, MaintenanceLog, Admin, VisionLog, Alert, EventLog,
)
from schemas.schemas import (
    ExtinguisherCreate, ExtinguisherUpdate,
    ExtinguisherView, ExtinguisherModelOut,
...
@router.get("/models", response_model=list[ExtinguisherModelOut])
def list_extinguisher_models(
    _: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    return db.query(ExtinguisherModel).order_by(ExtinguisherModel.model_id).all()
...
def update_extinguisher(
    ...
    if body.x_coord is not None or body.y_coord is not None:
        node = (
            db.query(FloorNode).filter(FloorNode.floor_node_id == e.node_id).first()
            if e.node_id else None
        )
        if node:
            ...
            if body.y_coord is not None:
                node.y = body.y_coord
        elif body.x_coord is not None and body.y_coord is not None:
            # FloorNode가 없거나(또는 참조가 끊어졌으면) 새로 생성 후 연결
...
def delete_extinguisher(
    ...
    log_ids = [
        row[0]
        for row in db.query(SensorLog.log_id).filter(SensorLog.extinguisher_id == ext_id).all()
    ]
    if log_ids:
        db.query(SensorData).filter(SensorData.log_id.in_(log_ids)).delete(synchronize_session=False)
    db.query(VisionLog).filter(VisionLog.extinguisher_id == ext_id).delete(synchronize_session=False)
    db.query(Alert).filter(Alert.extinguisher_id == ext_id).delete(synchronize_session=False)
    db.query(EventLog).filter(EventLog.extinguisher_id == ext_id).delete(synchronize_session=False)
    db.query(MaintenanceLog).filter(MaintenanceLog.extinguisher_id == ext_id).delete(synchronize_session=False)
    db.query(SensorLog).filter(SensorLog.extinguisher_id == ext_id).delete(synchronize_session=False)
...
def add_maintenance(
    ...
    admin_id = body.admin_id if body.admin_id is not None else (int(sub) if sub else None)
